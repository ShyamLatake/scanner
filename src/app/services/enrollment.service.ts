...
export class EnrollmentService {
   ...
   */
  initializeEnrollment(request: InitEnrollmentRequest): Observable<InitEnrollmentResponse> {
    const url = `${this.API_BASE_URL}/enroll/start-enrollment`;
    ...
    return this.http.post<InitEnrollmentResponse>(url, request, { headers })
      .pipe(
  ...
  uploadFrame(request: FrameUploadRequest): Observable<FrameUploadResult> {
    const url = `${this.API_BASE_URL}/enroll/enroll-frame`;
    ...
    formData.append('image', blob, `${request.pose.toLowerCase()}_${Date.now()}.jpg`);
    ...
    return this.http.post<FrameUploadResult>(url, formData)
      .pipe(
        timeout(this.REQUEST_TIMEOUT),
        catchError(this.handleError)
      );
  ...
  getEnrollmentStatus(sessionId: string): Observable<EnrollmentSession> {
    const url = `${this.API_BASE_URL}/enroll/enrollment-status/${sessionId}`;
    
    return this.http.get<EnrollmentSession>(url)
      .pipe(
        timeout(this.REQUEST_TIMEOUT),
        catchError(this.handleError)
      );
   ...
   */
  completeEnrollment(sessionId: string): Observable<{ success: boolean; message: string }> {
    const url = `${this.API_BASE_URL}/enroll/complete`;
    ...
    return this.http.post<{ success: boolean; message: string }>(url, body, { headers })
      .pipe(
        timeout(this.REQUEST_TIMEOUT),
        catchError(this.handleError)
      );
   ...
   */
  cancelEnrollment(sessionId: string): Observable<{ success: boolean; message: string }> {
    const url = `${this.API_BASE_URL}/enroll/cancel-enrollment`;
  ...
  getChildEmbeddings(childId: string): Observable<any> {
    const url = `${this.API_BASE_URL}/enroll/child/${childId}/embeddings`;
  ...
  private handleError = (error: any): Observable<never> => {
    ...
    if (error.name === 'TimeoutError') {
      ...
      errorMessage = error.error?.message || error.message || 'Client error occurred';
